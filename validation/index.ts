...
export const PostFormSchema = z.object({
  ...
  cityId: z.string().min(1, { message: "도시를 선택해주세요." }).trim(),
  districtId: z.string().min(1, { message: "행정구역을 선택해주세요." }).trim(),
  countryId: z.string().min(1, { message: "국가를 선택해주세요." }).trim(),
  image: z.string().min(1, { message: "썸네일을 업로드해주세요." }).trim(),
  location: z
    .object({
      lat: z
        .number()
